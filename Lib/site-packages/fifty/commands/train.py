...
from fifty.utilities.framework import build_model, build_autoencoder
from fifty.utilities.utils import json_paramspace2hyperopt_paramspace, dict_to_safe_filename
...
class Train:
    def __init__(self, options, *args):
        ...
        types_dict = {'dense': int, 'embed_size': int, 'filter': int, 'kernel': int, 'layers': int,
                     'pool': int, 'accuracy': float, 'loss': float, }
    ...
    def train_network(self, parameters, epochs=2, load=False):
        ...
        try:
            if load and os.path.exists(self.model_dir('.h5', params=parameters)):  # load old model
            ...
            callbacks_list = [
                callbacks.EarlyStopping(monitor=monitor, patience=50, restore_best_weights=True, min_delta=0.01),
            ...
            try:
                ...
                keras.utils.plot_model(model, os.path.join(self.model_dir(None, params=parameters), split_model_path[size-1] + '.png'), show_shapes=True)
            except Exception as e:
    ...
    def train_model(self):
        ...
        #if 'encoder' in self.best_hparams and self.best_hparams['encoder'] == 0.0:
        #    del self.best_hparams['encoder']
    ...
    def model_dir(self, ext=None, params=None) -> str:
        ...
        if params is not None:
            ...
            if 'encoder' in params and params['encoder'] != 0.0:
                params['encoder'] = os.path.split(params['encoder'])[-1]
            for k, v in params.items():
                if type(v) is float:
                    params[k] = int(v)
                if type(v).__module__ == np.__name__ or isinstance(v, tuple):
